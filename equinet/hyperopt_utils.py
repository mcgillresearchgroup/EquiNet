...
from equinet.constants import HYPEROPT_SEED_FILE_NAME
...
def build_search_space(search_parameters: List[str], train_epochs: int = None) -> dict:
    ...
    available_spaces = {
        "activation": hp.choice(
            "activation", options=["ReLU", "LeakyReLU", "PReLU", "tanh", "SELU", "ELU", "SiLU", "softplus"]
        ),
        "vp": hp.choice("vp", options=[None, "antoine", "simplified"]), #new
        "vle": hp.choice("vle", options=["activity", "basic", "nrtl", "nrtl-wohl", "wohl"]), #new
        "wohl_order": hp.quniform("wohl_order", low=3, high=5, q=1), #new
        "self_activity_correction": hp.choice("self_activity_correction", options=[True, False]), #new
        "self_activity_lambda": hp.choice(
            "self_activity_lambda", options=[
                hp.choice("no_self_activity_lambda",[0.]),
                hp.loguniform("self_activity_lambda_magnitude", low=np.log(1e-5), high=np.log(1e-1))]), #new
        "fugacity_balance":hp.choice("fugacity_balance", options=[True, False]), #new        
        ...
        "weight_decay": hp.loguniform("weight_decay", low=np.log(1e-6), high=np.log(1e-1)),
    ...
    for i, trial_dir in enumerate(manual_trials_dirs):
        ...
        for key in param_keys:
            if key == "init_lr_ratio":
                ...
                param_value = trial_args[key]
                val_value = ["ReLU", "LeakyReLU", "PReLU", "tanh", "SELU", "ELU", "SiLU", "softplus"].index(
                    param_value
                )
            else:
