...
class CommonArgs(Tap):
    ...
    def features_scaling(self) -> bool:
        """
        Whether to apply normalization with a :class:`~equinet.data.scaler.StandardScaler`
...
class TrainArgs(CommonArgs):
    """:class:`TrainArgs` includes :class:`CommonArgs` along with additional arguments used for training a EquiNet model."""
    ...
    with EquiNet's default molecule or multi-molecule encoders, or in :code:`reaction_solvent` mode where it applies to the solvent only.
    """
    ...
    self_activity_lambda: float = 0
    """A regularization parameter for reducing self-activity in the binary vle model."""
    uniquac_z: int = 10
    ...
    def atom_constraints(self) -> List[bool]:
        ...
        if self.is_atom_bond_targets and self.constraints_path:
            if not self._atom_constraints:
                header = equinet.data.utils.get_header(self.constraints_path)
    ...
    def process_args(self) -> None:
        ...
        self.smiles_columns = equinet.data.utils.preprocess_smiles_columns(
            path=self.data_path,
        ...
        if self.is_atom_bond_targets:
            self.atom_targets, self.bond_targets, self.molecule_targets = equinet.data.utils.get_mixed_task_names(
                path=self.data_path,
                smiles_columns=self.smiles_columns,
...
class PredictArgs(CommonArgs):
    """:class:`PredictArgs` includes :class:`CommonArgs` along with additional arguments used for predicting with a EquiNet model."""
...
class InterpretArgs(CommonArgs):
    """:class:`InterpretArgs` includes :class:`CommonArgs` along with additional arguments used for interpreting a trained EquiNet model."""
...
class HyperoptArgs(TrainArgs):
    """:class:`HyperoptArgs` includes :class:`TrainArgs` along with additional arguments used for optimizing EquiNet hyperparameters."""
