...
from equinet.constants import MODEL_FILE_NAME
from equinet.data import get_class_sizes, get_data, MoleculeDataLoader, MoleculeDataset, set_cache_graph, split_data
from equinet.models import MoleculeModel
from equinet.nn_utils import param_count, param_count_all
from equinet.utils import build_optimizer, build_lr_scheduler, load_checkpoint, makedirs, \
...
def run_training(args: TrainArgs,
    ...
    """
    Loads data, trains a EquiNet model, and returns test scores for the model checkpoint with the highest validation score.

    :param args: A :class:`~equinet.args.TrainArgs` object containing arguments for
                 loading data and training the EquiNet model.
    :param data: A :class:`~equinet.data.MoleculeDataset` containing the data.
    ...
    if len(val_data) == 0:
        raise ValueError('The validation data split is empty. During normal equinet training (non-sklearn functions), \
