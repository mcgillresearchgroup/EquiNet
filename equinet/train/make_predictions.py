...
def load_model(args: PredictArgs, generator: bool = False):
    ...
    :param args: A :class:`~equinet.args.PredictArgs` object containing arguments for
...
def load_data(args: PredictArgs, smiles: List[List[str]]):
    ...
    :return: A tuple of a :class:`~equinet.data.MoleculeDataset` containing all datapoints, a :class:`~equinet.data.MoleculeDataset` containing only valid datapoints,
                 a :class:`~equinet.data.MoleculeDataLoader` and a dictionary mapping full to valid indices.
    """
    print("Loading data")
...
def set_features(args: PredictArgs, train_args: TrainArgs):
    ...
    :param args: A :class:`~equinet.args.PredictArgs` object containing arguments for
                 loading data and a model and making predictions.
    :param train_args: A :class:`~equinet.args.TrainArgs` object containing arguments for training the model.
    """
...
def predict_and_save(
    ...
    :param args: A :class:`~equinet.args.PredictArgs` object containing arguments for
                 loading data and a model and making predictions.
    :param train_args: A :class:`~equinet.args.TrainArgs` object containing arguments for training the model.
    :param test_data: A :class:`~equinet.data.MoleculeDataset` containing valid datapoints.
    :param task_names: A list of task names.
    :param num_tasks: Number of tasks.
    :param test_data_loader: A :class:`~equinet.data.MoleculeDataLoader` to load the test data.
    :param full_data:  A :class:`~equinet.data.MoleculeDataset` containing all (valid and invalid) datapoints.
    :param full_to_valid_indices: A dictionary dictionary mapping full to valid indices.
    :param models: A list or generator object of :class:`~equinet.models.MoleculeModel`\ s.
    :param scalers: A list or generator object of :class:`~equinet.features.scaler.StandardScaler` objects.
    :param num_models: The number of models included in the models and scalers input.
    :param calibrator: A :class: `~equinet.uncertainty.UncertaintyCalibrator` object, for use in calibrating uncertainty predictions.
...
def equinet_predict() -> None:
    """Parses EquiNet predicting arguments and runs prediction using a trained EquiNet model.

    This is the entry point for the command line command :code:`equinet_predict`.
    """
