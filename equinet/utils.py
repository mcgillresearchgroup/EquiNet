...
from equinet.args import PredictArgs, TrainArgs, FingerprintArgs
...
def save_checkpoint(
    ...
    :param scaler: A :class:`~equinet.data.scaler.StandardScaler` fitted on the data.
    :param features_scaler: A :class:`~equinet.data.scaler.StandardScaler` fitted on the features.
    :param atom_descriptor_scaler: A :class:`~equinet.data.scaler.StandardScaler` fitted on the atom descriptors.
    :param bond_descriptor_scaler: A :class:`~equinet.data.scaler.StandardScaler` fitted on the bond descriptors.
    :param atom_bond_scaler: A :class:`~equinet.data.scaler.AtomBondScaler` fitted on the atomic/bond targets.
    :param args: The :class:`~equinet.args.TrainArgs` object containing the arguments the model was trained with.
...
def load_checkpoint(
    ...
    :return: The loaded :class:`~equinet.models.model.MoleculeModel`.
    """
...
def load_scalers(
    ...
    :return: A tuple with the data :class:`~equinet.data.scaler.StandardScaler`
             and features :class:`~equinet.data.scaler.StandardScaler`.
    """
    state = torch.load(path, map_location=lambda storage, loc: storage, weights_only=False)
...
def load_args(path: str) -> TrainArgs:
    ...
    :param path: Path where model checkpoint is saved.
    :return: The :class:`~equinet.args.TrainArgs` object that the model was trained with.
    """
    args = TrainArgs()
    args.from_dict(
...
def build_optimizer(model: nn.Module, args: TrainArgs) -> Optimizer:
    ...
    :param args: A :class:`~equinet.args.TrainArgs` object containing optimizer arguments.
...
def build_lr_scheduler(
    ...
    :param args: A :class:`~equinet.args.TrainArgs` object containing learning rate arguments.
...
def save_smiles_splits(
    ...
    :param train_data: Train :class:`~equinet.data.data.MoleculeDataset`.
    :param val_data: Validation :class:`~equinet.data.data.MoleculeDataset`.
    :param test_data: Test :class:`~equinet.data.data.MoleculeDataset`.
...
def update_prediction_args(
    ...
    :param predict_args: The :class:`~equinet.args.PredictArgs` object containing the arguments to use for making predictions.
    :param train_args: The :class:`~equinet.args.TrainArgs` object containing the arguments used to train the model previously.
    :param missing_to_defaults: Whether to replace missing training arguments with the current defaults for :class: `~equinet.args.TrainArgs`.
    ...
    if metric in scale_dependent_metrics:
        ...
        raise NotImplementedError(
            f"The metric used, {metric}, has not been added to the list of\
                ...
                function in `equinet/utils.py` in order to be used."
        )
