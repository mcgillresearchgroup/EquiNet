...
from equinet.features import get_features_generator
from equinet.features import BatchMolGraph, MolGraph
from equinet.features import is_explicit_h, is_reaction, is_adding_hs, is_mol, is_keeping_atom_map
from equinet.rdkit import make_mol
...
def cache_graph() -> bool:
    r"""Returns whether :class:`~equinet.features.MolGraph`\ s will be cached."""
    return CACHE_GRAPH


def set_cache_graph(cache_graph: bool) -> None:
    r"""Sets whether :class:`~equinet.features.MolGraph`\ s will be cached."""
    global CACHE_GRAPH
    CACHE_GRAPH = cache_graph


def empty_cache():
    r"""Empties the cache of :class:`~equinet.features.MolGraph` and RDKit molecules."""
...
class MoleculeDataset(Dataset):
    ...
    def batch_graph(self) -> List[BatchMolGraph]:
        r"""
        Constructs a :class:`~equinet.features.BatchMolGraph` with the graph featurization of all the molecules.

        .. note::
           The :class:`~equinet.features.BatchMolGraph` is cached in after the first time it is computed
           and is simply accessed upon subsequent calls to :meth:`batch_graph`. This means that if the underlying
           set of :class:`MoleculeDatapoint`\ s changes, then the returned :class:`~equinet.features.BatchMolGraph`
           will be incorrect for the underlying data.

        :return: A list of :class:`~equinet.features.BatchMolGraph` containing the graph featurization of all the
    ...
    def normalize_features(self, scaler: StandardScaler = None, replace_nan_token: int = 0,
        ...
        """
        Normalizes the features of the dataset using a :class:`~equinet.data.StandardScaler`.

        The :class:`~equinet.data.StandardScaler` subtracts the mean and divides by the standard deviation
        for each feature independently.

        If a :class:`~equinet.data.StandardScaler` is provided, it is used to perform the normalization.
        Otherwise, a :class:`~equinet.data.StandardScaler` is first fit to the features in this dataset
        and is then used to perform the normalization.

        :param scaler: A fitted :class:`~equinet.data.StandardScaler`. If it is provided it is used,
                       otherwise a new :class:`~equinet.data.StandardScaler` is first fitted to this
        ...
        :return: A fitted :class:`~equinet.data.StandardScaler`. If a :class:`~equinet.data.StandardScaler`
                 is provided as a parameter, this is the same :class:`~equinet.data.StandardScaler`. Otherwise,
                 this is a new :class:`~equinet.data.StandardScaler` that has been fit on this dataset.
        """
    ...
    def custom_normalize_hybrid_features(self, target_scaler: StandardScaler, replace_nan_token: int = 0,
        ...
        for i, j in matched_scaling_pairs:
            scaler.means[j] = scaler.means[i]
            scaler.stds[j] = scaler.stds[i]
    ...
    def normalize_targets(self, unscaled_target_indices=[], offset_only_indices=[], no_offset_indices=[], matched_scaling_pairs=[]) -> StandardScaler:
        """
        Normalizes the targets of the dataset using a :class:`~equinet.data.StandardScaler`.
        The :class:`~equinet.data.StandardScaler` subtracts the mean and divides by the standard deviation
        for each task independently.
        This should only be used for regression datasets.
        :return: A :class:`~equinet.data.StandardScaler` fitted to the targets.
        """
        targets = np.array([d.raw_targets for d in self._data], dtype=np.float64)
        scaler = StandardScaler().fit(targets)

        for i in offset_only_indices:
            scaler.means[i] = np.nanmean(targets[:, i])
            scaler.stds[i] = 1
        for i in no_offset_indices:
            scaler.means[i] = 0
            scaler.stds[i] = np.nanmean(targets[:, i])
    ...
    def normalize_atom_bond_targets(self) -> AtomBondScaler:
        """
        Normalizes the targets of the dataset using a :class:`~equinet.data.AtomBondScaler`.

        The :class:`~equinet.data.AtomBondScaler` subtracts the mean and divides by the standard deviation
        for each task independently.

        This should only be used for regression datasets.

        :return: A :class:`~equinet.data.AtomBondScaler` fitted to the targets.
        """
...
def construct_molecule_batch(data: List[MoleculeDatapoint]) -> MoleculeDataset:
    ...
    Additionally, precomputes the :class:`~equinet.features.BatchMolGraph` for the constructed
