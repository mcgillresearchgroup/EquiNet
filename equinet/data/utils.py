...
from equinet.features import load_features, load_valid_atom_or_bond_features, is_mol
...
def filter_invalid_smiles(data: MoleculeDataset) -> MoleculeDataset:
    ...
    :param data: A :class:`~equinet.data.MoleculeDataset`.
    :return: A :class:`~equinet.data.MoleculeDataset` with only the valid molecules.
    """
...
def get_data(path: str,
    ...
    :param args: Arguments, either :class:`~equinet.args.TrainArgs` or :class:`~equinet.args.PredictArgs`.
    ...
    :param store_row: Whether to store the raw CSV row in each :class:`~equinet.data.data.MoleculeDatapoint`.
    ...
    :return: A :class:`~equinet.data.MoleculeDataset` containing SMILES and target values along
    ...
    with open(path) as f:
        ...
        if any([c not in fieldnames for c in target_columns]) and args.vle is None:
        ...
        all_smiles, all_targets, all_rows, all_features, all_weights, all_hybrid_model_features = [], [], [], [], [], []
        for i, row in enumerate(tqdm(reader)):
            ...
            if args.vle is not None:
                # fill targets list with None for the rest of the 7 vle outputs: x1, x2, log10P, ln_gamma1, ln_gamma2, log10p1sat, log10p2sat
                targets += [None] * (7 - len(targets))
        ...
        if args.vle is not None: # x1, x2, T, log10p1sat, log10p2sat
            all_hybrid_model_features = all_features # x1, x2, T, log10p1sat, log10p2sat
        ...
        if args.vle is not None: # x1, x2, T, log10p1sat, log10p2sat
            ...
            elif args.vle in ["activity", "freestyle"]:
...
def get_data_from_smiles(smiles: List[List[str]],
    ...
    """
    Converts a list of SMILES to a :class:`~equinet.data.MoleculeDataset`.
    ...
    :return: A :class:`~equinet.data.MoleculeDataset` with all of the provided SMILES.
    """
    debug = logger.debug if logger is not None else print
...
def split_data(data: MoleculeDataset,
    ...
    :param args: A :class:`~equinet.args.TrainArgs` object.
    :param logger: A logger for recording output.
    :return: A tuple of :class:`~equinet.data.MoleculeDataset`\ s containing the train,
...
def get_class_sizes(data: MoleculeDataset, proportion: bool = True) -> List[List[float]]:
    ...
    :param data: A classification :class:`~equinet.data.MoleculeDataset`.
