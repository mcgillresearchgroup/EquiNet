...
def squared_log_fugacity_difference(
        ...
        masks: torch.tensor,
) -> torch.tensor: #TODO fix this loss
    """
    ...
    """
    # masks: y1, y2, log10P, ln_gamma_1, ln_gamma_2, log10p1sat, log10p2sat
    ln_gamma_1 = pred_values[:,[3]]
    ln_gamma_2 = pred_values[:,[4]]
    log10p1sat = pred_values[:,[5]]
    log10p2sat = pred_values[:,[6]]

    y1 = targets[:,[0]]
    y2 = targets[:,[1]]
    log10P = targets[:,[2]]
    target_ln_gamma_1 = targets[:,[3]]
    target_ln_gamma_2 = targets[:,[4]]
    ...
    log10_gamma_1 = ln_gamma_1 / np.log(10)
    log10_gamma_2 = ln_gamma_2 / np.log(10)
    target_log10_gamma_1 = target_ln_gamma_1 / np.log(10)
    target_log10_gamma_2 = target_ln_gamma_2 / np.log(10)
    log10_x1 = torch.log10(x1)
    log10_x2 = torch.log10(x2)
    log10_y1 = torch.log10(y1)
    log10_y2 = torch.log10(y2)

# if gamma is provided as a target, use the gamma losses
# if x or y is 0, then don't use regular losses
# but you would still want it for pure 1 for vapor pressure

    loss_1 = (log10_x1 + log10_gamma_1 + log10p1sat - log10_y1 - log10P) ** 2
    loss_2 = (log10_x2 + log10_gamma_2 + log10p2sat - log10_y2 - log10P) ** 2
    loss_g1 = (log10_gamma_1 - target_log10_gamma_1) ** 2
    loss_g2 = (log10_gamma_2 - target_log10_gamma_2) ** 2
    loss = torch.cat([loss_1, loss_2, loss_g1, loss_g2], dim=1)
